...
class TableController extends BaseController {
    ...
    public function crud(Request $request): Response {
        ...
        $pid = (int)$pid;
        if ($pid) {
            $parent_menu = RuleModel::find($pid);
            if (!$parent_menu) {
        ...
        $menu = RuleModel::where('key', $controller_class_with_namespace)->first();
        if (!$menu) {
            $menu = new RuleModel;
        }
        $menu->pid = $pid;
        ...
        if (!in_array('*', $rule_ids) && $roles) {
            $role = RoleModel::find(current($roles));
            if ($role) {
                $role->rules .= ",{$menu->id}";
            }
            $role->save();
...
EOF;
    ...
    protected function modifyColumn($column, $table) {
        ...
        if ($old_field && $old_field !== $field) {
            $sql = sprintf("ALTER TABLE `%s` CHANGE COLUMN `%s` `%s` ", $table, $old_field, $field);
        } else {
            $sql = sprintf("ALTER TABLE `%s` MODIFY `%s` ", $table, $field);
        }
    ...
    protected function updateSchemaOption($table_name, $data): string {
        ...
        $option = OptionModel::where('name', $option_name)->first();
        if ($option) {
            OptionModel::where('name', $option_name)->update(['value' => $data]);
        } else {
            OptionModel::insert(['name' => $option_name, 'value' => $data]);
        }
        return $option_name;
    }
