...
//    protected $dataLimit = 'auth';
    ...
    public function update(Request $request): Response {
        ...
        $before_data = $this->model->select(['category', 'name'])->find($id)->toArray();
        $this->doUpdate($id, $data);
        LogModel::saveLog(
            LogModel::OP_USER_TYPE_ADMIN,
            LogModel::OP_TYPE_ATTACHMENT,
            $id,
            $before_data,
            $data
        );
    ...
    public function insert(Request $request): Response {
        ...
        $upload->admin_id = AdminModel::adminId();
        ...
        return $this->success('success_upload');
    }
    ...
    public function image(Request $request): Response {
        ...
        try {
            ...
            unlink($realpath);
            throw new BusinessException(trans('error_image_processing'));
        }
        $this->output = [
            'url' => $data['url'],
            'name' => $data['name'],
    ...
    public function avatar(Request $request): Response {
        ...
        if ($file && $file->isValid()) {
            ...
            $path = $base_path . "/$relative_path/$name.lg.$ext";
            $image->save($path);

            $image->resize(120, 120);
            $path = $base_path . "/$relative_path/$name.md.$ext";
            $image->save($path);

            $image->resize(60, 60);
            $path = $base_path . "/$relative_path/$name.$ext";
            $image->save($path);

            $image->resize(30, 30);
            $path = $base_path . "/$relative_path/$name.sm.$ext";
            $image->save($path);

            $this->output['url'] = "/$relative_path/$name.md.$ext";
            $this->success('success_upload');
        }
        return $this->error('error_file_not_found');
    }
    ...
    public function delete(Request $request): Response {
        ...
        $files = $this->model->whereIn('id', $ids)->get()->keyBy('id')->toArray();
        $before_data = $files;
        $file_list = array_map(function ($item) {
            $path = $item['url'];
            if (preg_match("#^/upload#", $path)) {
                $admin_public_path = config('public_path') ?: base_path() . "/public";
                return $admin_public_path . $item['url'];
            }
            return null;
        }, $files);
...
//        $admin_public_path = rtrim(config('public_path', '/public'), '\\/');
//        loginfo('$admin_public_path' . $admin_public_path);
//        $base_dir = $admin_public_path ? $admin_public_path . DIRECTORY_SEPARATOR : base_path() . '/';
        $base_dir = base_path() . '/public/';
