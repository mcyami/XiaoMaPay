...
class DictModel {
    ...
    public static function get($name) {
        $value = OptionModel::where('name', static::dictNameToOptionName($name))->value('value');
    ...
    public static function save($name, $values) {
        if (!preg_match('/[a-zA-Z]/', $name)) {
            throw new BadRequestHttpException('字典名只能包含字母');
        }
        $option_name = static::dictNameToOptionName($name);
        if (!$option = OptionModel::where('name', $option_name)->first()) {
    ...
    public static function delete(array $names) {
        ...
        OptionModel::whereIn('name', $names)->delete();
    }
    ...
    public static function filterValue(array $values): array {
        ...
        foreach ($values as $item) {
            if (!isset($item['value']) || !isset($item['name'])) {
                throw new BadRequestHttpException('字典格式错误');
            }
